NAME                = ocm-food-app
PROVIDER            ?= mohammad-ammar14
WW_DEMOS_PROVIDER   ?= ammardemos
IMAGE               = $(NAME)
COMPONENT           = github.com/$(PROVIDER)/$(NAME)
OCI_REPO_BASE       ?= ghcr.io/$(PROVIDER)
OCI_REPO            ?= ghcr.io/mohammad-ammar14/ocm-food-app
KEY                 ?= rsa.priv
SIGN_NAME           ?= ci-keypair

CHILD_COMPONENTS    ?= carts
GEN_DIRS            := $(addprefix $(CHILD_COMPONENTS),/component-archive)
